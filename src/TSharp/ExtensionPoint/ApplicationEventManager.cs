...
namespace TSharp.Core.Osgi
{
  ...
  {
    private static readonly ConcurrentDictionary<RegAppEventAttribute, IAppHandler> HandlerTypes = new();
    ...
    protected internal override void OnInit()
    {
      ...
      foreach (var key in HandlerTypes.Keys)
      {
        ...
        {
          log.LogError(ex, "Osgi:创建AppHandler时异常！类型:'{0}'", key.EventType.FullName);
        }
      }
    }
    protected internal override void OnLoad()
    {
      ...
      foreach (var i in HandlerTypes.Values)
      {
        ...
        {
          log.LogError(ex, "Osgi:执行OnStart(AppEventArgs)时异常！类型:'{0}'", i.GetType().FullName);
        }
      }
    ...
    protected internal override void UnLoad()
    {
      ...
      foreach (var i in HandlerTypes.Values)
      {
        ...
        {
          log.LogError(ex, "Osgi:执行OnStop(AppEventArgs)时异常！类型:'{0}'", i.GetType().FullName);
        }
      }
    }
  }

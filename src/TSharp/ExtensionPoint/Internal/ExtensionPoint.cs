...
namespace TSharp.Core.Osgi.Internal
{
  ...
  public abstract class ExtensionPoint
  {
    ...
    //internal ExtensionPoint(ILogger log)
    //{
    //   this.log = log;
    //}

    internal List<OsgiEngine.RegExtensionAttributeItem> allReg = new(2000);
    ...
    internal virtual void UnRegisterAll()
    {
      foreach (var item in allReg)
        ...
        {
          log.LogError(ex, "程序集{0}中反注册属性{1}时发生错误。", ex, item.Assembly, item.ExtensionAttribute);
          if (OnErrorBreak(ex))
            throw;
        }
    }
